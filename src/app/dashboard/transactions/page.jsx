...
export default function TransactionsPage() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const mappedTransactions = res.data.map((item) => ({
        id: item.transactionId,
        user: item.buyerEmail,
        type: "Purchase",
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-3 w-full">
          ...
          <div className="relative w-full sm:w-44">
            <select
              ...
              className="w-full appearance-none bg-indigo-600 border border-[#2837c1] text-sm text-white px-4 py-2 rounded-lg focus:outline-none focus:border-[#5826df] cursor-pointer pr-10"
            >
