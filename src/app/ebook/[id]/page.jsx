...
export default function EbookDetailsPage() {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://localhost:5000/ebook/${id}`);
  ...
  // 🛠️ ডোমেইন ব্লক এরর এড়াতে স্ট্যান্ডার্ড এবং ওপেন সোর্স ফলব্যাক ইমেজ কভার সেট করা হলো
  const defaultBookCover = "https://co.com";
      ...
      <div className="grid lg:grid-cols-2 gap-10 items-start pb-20">
        ...
        <div className="text-gray-800 bg-white" style={{ padding: "8px" }}>
          ...
          <div className="flex flex-wrap gap-4 mt-10">
            <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-2xl font-semibold transition-all shadow-lg shadow-indigo-600/30 cursor-pointer" style={{ padding: "3px" }}>
              Buy Now
            </button>

            <button className="border border-indigo-500/40 text-white bg-indigo-500 hover:bg-indigo-500/10  rounded-2xl flex items-center gap-2 transition-all cursor-pointer" style={{ padding: "3px" }}>
              <Heart size={18} />
              Bookmark
            </button>
          </div>
